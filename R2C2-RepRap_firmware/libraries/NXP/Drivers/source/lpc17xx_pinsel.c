...
static void set_PinFunc ( uint8_t portnum, uint8_t pinnum, uint8_t funcnum)
{
	...
	*(__IO uint32_t *)(pPinCon + pinselreg_idx) &= ~(0x03UL << (pinnum_t * 2));
	*(__IO uint32_t *)(pPinCon + pinselreg_idx) |= ((uint32_t)funcnum) << (pinnum_t * 2);
}
...
void set_ResistorMode ( uint8_t portnum, uint8_t pinnum, uint8_t modenum)
{
	...
	__IO uint32_t *pPinCon = &LPC_PINCON->PINMODE0;
	...
	*(__IO uint32_t *)(pPinCon + pinmodereg_idx) &= ~(0x03UL << (pinnum_t * 2));
	*(__IO uint32_t *)(pPinCon + pinmodereg_idx) |= ((uint32_t)modenum) << (pinnum_t * 2);
}
...
{
	__IO uint32_t *pPinCon = &LPC_PINCON->PINMODE_OD0;

	if (modenum == PINSEL_PINMODE_OPENDRAIN){
		*(__IO uint32_t *)(pPinCon + portnum) |= (0x01UL << pinnum);
	} else {
		*(__IO uint32_t *)(pPinCon + portnum) &= ~(0x01UL << pinnum);
	}
}
